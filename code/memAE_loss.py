...
get_sl1_loss = torch.nn.SmoothL1Loss() 
...
def get_memory_loss(memory_att):
    ...
    memory_att = memory_att.sum() / s[0] #(s[0] * s[-2] * s[-1]) 
    return memory_att 
...
def get_reconstruction_loss(x_input, x_recons, mean=0.5, std=0.5):
    ...
    batch_size, ch = x_input.shape
    #x_input, x_recons = get_unormalized_data(x_input, x_recons, mean, std)
    recons_loss = get_sl1_loss(x_input, x_recons)
    #recons_loss = (x_input - x_recons) ** 2
    #recons_loss = recons_loss.sum().sqrt()/batch_size
    return recons_loss
